...
const generateContributionData = () => {
  ...
  for (let week = 0; week < weeks; week++) {
    ...
    for (let day = 0; day < days; day++) {
      ...
      if (week > 40) { // Most recent weeks - higher activity
        ...
        if (day > 0 && weekData[day-1] > 0 && Math.random() > 0.3) {
          intensity = Math.min(4, weekData[day-1] + Math.floor(Math.random() * 2));
        }
        // Weekends have less activity
        if ((day === 0 || day === 6) && Math.random() > 0.3) {
          intensity = 0;
        }
      } else if (week > 30) { // Medium activity period
        intensity = Math.random() > 0.7 ? Math.floor(Math.random() * 3) + 1 : 0;
      } else { // Earlier period - less activity
        intensity = Math.random() > 0.85 ? Math.floor(Math.random() * 2) + 1 : 0;
      }
...
const DataVisualization = ({ className = '' }) => {
  ...
  return (
    <div className={`bg-gray-900 rounded-lg shadow-xl overflow-hidden p-6 ${className}`}>
      <div className="mb-12">
        <h3 className="text-xl font-semibold text-gray-200 mb-6">GitHub Contributions</h3>
                    ...
                    {week.map((intensity, dayIndex) => {
                      ...
                      return (
                        <motion.div
                          ...
                          initial={{ opacity: 0 }}
                          animate={{ opacity: 1 }}
                          transition={{ duration: 0.01, delay: (weekIndex * 7 + dayIndex) * 0.001 }}
                          onMouseEnter={() => {
                            setHoverInfo({
                              ...
                              count: intensity === 0 ? 'No contributions' :
                                     intensity === 1 ? '1-2 contributions' :
                                     intensity === 2 ? '3-5 contributions' :
                                     intensity === 3 ? '6-9 contributions' :
                                     '10+ contributions',
      ...
      {/* Skills Bars */}
      <div className="mt-12">
        <h3 className="text-xl font-semibold text-gray-200 mb-6">Skill Proficiency</h3>
