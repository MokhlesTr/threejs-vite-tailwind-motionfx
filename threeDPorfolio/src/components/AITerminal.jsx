...
const AITerminal = ({ className = '', height = 400 }) => {
  ...
  useEffect(() => {
    const simulateTerminal = async () => {
      ...
      while (true) {
        // Reset for a new sequence of commands
        setLines([]);
        setProgress(0);
        
        // Add header to terminal
        await addLineWithTypingEffect('> AI Agent v3.7.2 initialized', colors.success, 30);
        await addLineWithTypingEffect('> Starting inference pipeline...', colors.text, 40);
        await new Promise(resolve => setTimeout(resolve, 800));
        
        // Run a random number of commands
        const numCommands = 3 + Math.floor(Math.random() * 5);
        
        for (let i = 0; i < numCommands; i++) {
          setProgress(i / numCommands);
          // Select random command
          const commandIndex = Math.floor(Math.random() * commands.length);
          const command = commands[commandIndex];
          
          // Type the command with a prompt
          await addLineWithTypingEffect(`$ ${command}`, colors.command, 20);
          
          // Wait a bit
          await new Promise(resolve => setTimeout(resolve, 400 + Math.random() * 300));
          
          // Show successful result
          await addLine(`✓ Complete`, colors.success);
        ...
        setProgress(1);
        await addLine('', '');
        await addLineWithTypingEffect('> Processing complete. Result:', colors.success, 30);
        ...
        // Wait a longer time before restarting
  ...
  const addLineWithTypingEffect = async (text, color, speed = 50) => {
    ...
    for (let i = 0; i < text.length; i++) {
      ...
      await new Promise(resolve => setTimeout(resolve, speed * (0.5 + Math.random() * 0.8)));
    }
      ...
      >
        ...
        >
          {lines.map((line, index) => (
            <div key={index} style={{ color: line.color }} className="whitespace-pre-wrap">
              {line.text}
            </div>
          ))}
          
          {isTyping && (
            <div style={{ color: currentLine.startsWith('$') ? colors.command : colors.text }}>
              {currentLine}
        ...
        <div 
          ...
          style={{
            background: 'linear-gradient(to bottom, transparent 0%, rgba(0, 30, 60, 0.15) 85%)',
            pointerEvents: 'none',
          }}
        />
      </div>
